package com.android.xuanhong.tryonglasses.models.controller;
...
import com.android.xuanhong.tryonglasses.models.view.ModelRenderer;
import com.android.xuanhong.tryonglasses.models.view.ModelSurfaceView;
...
public class TouchController {
		...
		if (touchDelay > 1) {
			...
			if (pointerCount == 1 && currentPress1 > 4.0f) {
				...
				if (fingersAreClosing) {
					...
					float zoomFactor = (length - previousLength) / ((mRenderer.getWidth() + mRenderer.getHeight())) * 30;
	...
	private void selectObjectImpl(float[] nearPoint, float[] farPoint) {
		LoaderScenes scene = view.getModelActivity().getScene();
		if (scene == null) {
			return;
		}
